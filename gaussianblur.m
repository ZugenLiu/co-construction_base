function timetostable = gaussianblur(sigma2)
close all

% function [sigma2,timetostable,change,viable]=gaussianblur(sigma2)
...
tk=100;%5000;        %how long it must remain stable
...
while tstable<=tk && viable==1
    ...
    if time==perturb
        ...
        MM = gauss2dconvolve(MM,sigma2);
        AA = gauss2dconvolve(AA,sigma2);
        WW = gauss2dconvolve(WW,sigma2);
        FF = gauss2dconvolve(FF,sigma2);
        
        save('sigmablur.mat','MM','AA','WW','FF','DIR');
...
strsave=(['sigma2=' num2str(sigma2)]);
save([strsave '.mat'],'MM','AA','WW','FF','DIR');

results=[sigma2 timetostable change viable];
% save(fullfile(savdir,str),'resultCell','-append')
% savdir = '/Users/eranagmon/Dropbox/autop_Matlab/autopoiesis_9-24/blur/blur.mat';
